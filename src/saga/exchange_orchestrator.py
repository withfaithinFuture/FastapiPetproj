...
class SaveExchangeOrchestrator:
    ...
    async def create_exchange_with_saga(self, exchange_data: ExchangeCreateSchema, exchange_key: str):
        ...
        try:
            ...
            raise LocalDBError(object_type="Exchange", object_id=exchange_dict['id'])

        try:
            logger_saga.info(f"Сага - транзакция во 2 сервис-клиент")
            additional_exchange_data = await self.second_client.create_additional_info(exchange_name=exchange_dict['exchange_name'])
            ...
            logger_saga.error(f"Сага - ошибка 2 сервиса")
            exchange.status = SagaStatus.FAILED

            await self.exchange_repo.update_object(exchange)
            await self.exchange_repo.session.commit()

            raise SagaTransactionError(service_name="Second_service")
